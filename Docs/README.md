...
Сборщик создаёт четыре штуки в `Assets/Weapons/`:

| Оружие | Тип огня | ЛКМ | ПКМ (альт-огонь) |
|---|---|---|---|
| **Револьвер** | **хитскан** (без снаряда, трассер) | мгновенный луч, ровный урон на любой дистанции; при полном заряде грейза — **мощный выстрел** (пробивает все цели, ×3 урона) | **монетка**: луч, попавший в монетку, рикошетит с **автонаводкой на приоритетную цель** (самый «жирный» враг) и доп. уроном |
...
- от **скорости** зависит **время жизни**: `life = baseLife × (speed / refSpeed)` — разогнанный мяч
  живёт дольше, затухающий испаряется;
- мяч **бьёт и игрока** (вполсилы) — швыряться им вслепую опасно;
- каждый отскок от стен/тел теряет немного скорости (`bounceKeep`).
